...
Веб-приложение CRM: **клиенты** и **заявки**. Бэкенд — Django REST, фронтенд — React. Авторизация по сессии (логин/пароль). Профили сотрудников с ФИО, телефоном, должностью и аватаркой.

## Возможности

- **Клиенты:** просмотр, поиск, добавление, редактирование, удаление (все авторизованные).
- **Заявки:** просмотр и поиск — все; **создание** — все; **редактирование и удаление** — только администратор. Этапы: В обработке, В работе, Контроль выполненных задач, Выполнено. У заявки можно указать несколько ответственных (настраивает админ).
- **Личный кабинет (/profile):** отображение данных профиля; смена аватарки — все пользователи; редактирование ФИО, телефона, должности — только админ.
- **Регистрация сотрудника (/register):** только для админа; отдельная страница с полями логин, пароль, ФИО, телефон, должность, аватар.
- **Тема:** переключение светлой/тёмной темы в шапке и на странице входа; выбор сохраняется в браузере.
- **Админка Django:** ссылка в шапке (для админа); управление пользователями с профилями (ФИО, телефон, должность, аватар), заявками и клиентами.

## Стек

- **Бэкенд:** Python 3.12, Django 5.2, Django REST Framework, SQLite, Pillow (аватарки)
...
│   ├── backend/          # настройки (settings.py, urls.py, media)
│   ├── clients/          # приложение «Клиенты»
│   ├── deals/             # приложение «Заявки» (модель Deal, ответственные M2M)
│   ├── users/             # авторизация, профили (Profile: отчество, телефон, должность, аватар)
│   ├── manage.py
│   ├── db.sqlite3         # БД (не коммитить)
│   └── media/             # загруженные аватарки (создаётся при первой загрузке)
├── frontend/              # React (CRA)
│   ├── src/
│   │   ├── api.js         # запросы к API, FormData, прокси на бэкенд
│   │   ├── App.js         # маршруты: /, /login, /register, /profile
│   │   ├── setupProxy.js  # прокси /api и /media -> Django
│   │   └── components/    # Login, Dashboard, RegisterPage, ProfilePage, ProtectedRoute
│   └── package.json
...
**Страницы:** `/login` — вход (учётные данные как в админке Django); `/` — главная (вкладки «Заявки» и «Клиенты»); `/profile` — личный кабинет (по клику на логин в шапке); `/register` — регистрация нового сотрудника (только для администратора).
